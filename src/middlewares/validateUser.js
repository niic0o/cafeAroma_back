...
const loginValidation = joi.object({
  ...
  password: joi.string()
    ...
    .messages({
      ...
      "string.min": "El campo contraseña debe tener al menos {#limit} caracteres.",
      "string.max": "El campo contraseña no debe tener más de {#limit} caracteres.",
...
const createUserValidation = joi.object({
  ...
  domicilio: joi.string().min(6).max(100).required().messages({
    "string.base": "El campo domicilio debe ser un texto.",
    "string.empty": "El campo domicilio es requerido.",
    "string.min": "El campo domicilio debe tener al menos {#limit} caracteres.",
    "string.max":
  ...
  .error(errors => {
    errors.forEach(err => {
      if (err.code === 'number.base') {
        err.message = "El campo DNI debe ser un número entero y no una cadena.";
      }
    });
    return errors;
  }),
